...
lemma interList_get_succ_eq_get_of_equiv_punit (i : Fin s.length)
  ...
  replace uniq_qf := Submodule.unique_quotient_iff_eq_top.mp uniq_qf
  ext x : 1; fconstructor
  · intro hx; exact (eq_top_iff.mp uniq_qf) (by trivial : ⟨x, hx⟩ ∈ ⊤)
...
lemma eq_interList_get_of_head_eq_bot_and_interList_nodup (s0 : s.head = ⊥)
  ...
  · rw [← H]
    ...
    simp only [Submodule.mem_inf, Fin.cast_mk, List.get_eq_getElem, Submodule.mem_map,
      Submodule.coe_subtype, Subtype.exists, exists_and_right, exists_eq_right]
...
lemma eq_top_of_interList_nodup (s0 : s.head = ⊥) (slast : s.last = ⊤)
  ...
  simp only [List.get_eq_getElem, Fin.coe_cast, Fin.val_last] at eq0
...
lemma ofInterList_head_eq_bot_of_head_eq_bot
  ...
  rw [this]
  exact interList_head_eq_bot_of_head_eq_bot (s0 := s0) _ _
...
lemma exists_compositionSeries_with_smaller_length_of_lt_top (h : N < ⊤)
  ...
  have ineq2 : 0 < (s.interList N).dedup.length := by
    ...
    push_neg at rid
    norm_num at rid
    exact List.map_ne_nil_of_ne_nil _ s.toList_ne_nil _ rid
