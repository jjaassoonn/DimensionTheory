...
theorem coeff_mul_invOneSubPow_eq_hilbert_eval (p : ℤ[X]) (d n : ℕ) (hn : p.natDegree < n) :
  ...
  · simp only [h, ↓reduceDite, zsmul_eq_mul]
    ...
    · simp only [h1, ↓reduceIte, eval_zero, Int.cast_eq_zero]
      ...
      erw [Units.inv_mul (invOneSubPow d : ℤ⟦X⟧ˣ)]
      rw [mul_one]
      rw [← coe_one_sub, ← coe_pow, ← coe_mul, coeff_coe]
    ...
    · simp only [h1, ↓reduceIte]
      ...
      rw [← mul_pow]
      erw [Units.mul_inv (Units.mkOfMulEqOne (1 - PowerSeries.X : ℤ⟦X⟧) (mk 1) _)]
      rw [one_pow, one_mul]
      ...
      rw [eval_finset_sum]
      simp only [eval_mul, eval_intCast]
